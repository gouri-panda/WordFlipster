...
fun WordCrypticMainScreen(navController: NavController, category: String){
    BaseScreen {
        ...
        homeViewModel.updateCurrentScreen(Screens.WordCrypticMainScreen)
        if (showDialog) {
            AlertDialog(
        ...
        if (!homeViewModel.isLoading.value) {
            Box(
                ...
                Column(
                    ...
                    val currentScreen =
                        remember { mutableStateOf(homeViewModel.currentScreen.value) }
                    ...
                    Box(
                        ...
                        Column(
                            ...
                            WordGridWithWoodenTiles(charList)
                            ArrowButton()
    ...
    WordFlipsterTheme {
        ...
        Row(
            ...
            InputWordView(
                ...
                onValueChange = { value ->
                    if (value.length <= crypticWord.size) {
                        Log.d("Actual Value", value)
                        if (value.length == correctWord.size) {
...
fun encodeWordWithRandomShift(word: String): Pair<String, Int> {
    val randomShift = generateNonZeroShift(-5, 5)
    val encodedWord = word.map { char ->
        if (char.isLetter()) {
            val base = if (char.isLowerCase()) 'a' else 'A'
            ((char - base + randomShift + 26) % 26 + base.code).toChar()
        } else char
    }.joinToString("")
    return encodedWord to randomShift
}

// Helper function to ensure non-zero shifts
fun generateNonZeroShift(min: Int, max: Int): Int {
    var shift: Int
    do {
        shift = Random.nextInt(min, max + 1) // Include max value
    } while (shift == 0)
    return shift
}
fun decodeWord(encodedWord: String, shift: Int): String {
    return encodedWord.map { char ->
        if (char.isLetter()) {
            val base = if (char.isLowerCase()) 'a' else 'A'
